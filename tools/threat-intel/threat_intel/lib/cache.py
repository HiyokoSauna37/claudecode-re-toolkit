"""SQLite-backed result cache with @cached decorator.

Stores at ~/.threat_intel_cache.db. TTL controlled by configvars.cache_ttl
(default 3600s). Use @cached("prefix") on _raw_* methods.
"""
...
class ResultCache:
    def __init__(self, db_path=None, default_ttl=3600):
        if db_path is None:
            db_path = os.path.join(str(Path.home()), '.threat_intel_cache.db')
...
def cached(key_prefix):
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            parts = [key_prefix] + [str(a) for a in args[1:]]
            ...
            # Don't cache error dicts — transient errors (network, missing key)
