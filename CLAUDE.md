...
静的解析（Ghidra Headless）、動的解析（VMware Sandbox）、Webフォレンジック（Proxy Web）、脅威インテリ問合せ（threat-intel）の4つのスキルを統合し、Claude Codeからワンストップでマルウェア解析を実行できる。
...
#### malware-sandbox（VMware Workstation必須）

[tools/malware-sandbox/docs/VM-SETUP.md](tools/malware-sandbox/docs/VM-SETUP.md) を参照してVM環境を構築。
...
```
1. malware-fetch でURLアクセス → バイナリ取得（暗号化）
2. ghidra-headless で静的解析 → パッカー/ファミリー判定
3. パック済み or 静的解析限界 → malware-sandbox で動的解析
4. アンパック後バイナリ → ghidra-headless で再解析
5. 平行: threat-intel で hash/IP/URL/CVE を相関問合せ → 帰属判定
```
...
**派生式は CLAUDE.md に書かない**。実際の導出 (`GUEST_ANALYSIS_DIR=${GUEST_PROFILE}\Desktop\analysis` など) は `.env.example` と `tools/malware-sandbox/sandbox.sh` が single source of truth。CLAUDE.md では placeholder の意味と方針のみを示し、実値は環境変数／既存ツールに委ねる。
