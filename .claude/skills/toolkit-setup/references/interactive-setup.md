...
- 用途説明: マルウェアファイルのAES-256暗号化パスワード。malware-fetch / ghidra-headless / malware-sandbox の全ツールで共通利用
- 選択肢: 「自動生成（推奨）」「自分で入力する」
- 自動生成: `openssl rand -base64 24` で生成 → ユーザーに表示して確認
- 手動入力: Other で自由入力

**Q-B: VIRUSTOTAL_API_KEY**
- 用途説明: malware-fetch のハッシュチェック・VT連携に必要。無料アカウントで取得可
...
## Step 2: Docker — malware-fetch イメージビルド
...
### 2-2. 既存イメージ確認
```bash
docker images malware-fetch-browser:latest --format "{{.ID}} {{.CreatedAt}}"
```
...
# --no-cache 選択時:
docker build --no-cache -t malware-fetch-browser:latest tools/malware-fetch/
```
`run_in_background: true` で実行。完了後に結果を報告。

### 2-4. 検証
```bash
docker run --rm malware-fetch-browser:latest echo "OK"
```
...
```
見つからない場合 → `tools/malware-sandbox/docs/VM-SETUP.md` の Step 1-2 を要約提示。
...
承認後: `bash tools/malware-sandbox/sandbox.sh setup-guest`

### 5-5. ネットワーク・接続確認
```bash
bash tools/malware-sandbox/sandbox.sh net-status
bash tools/malware-sandbox/sandbox.sh status
```
