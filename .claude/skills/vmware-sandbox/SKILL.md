...
instructions: |
  ...
  2. **`analyze` ワンコマンドを使う場合は `start` 不要**（analyze が revert → net-isolate → start を内部で自動処理）。手動フローを使う場合のみ停止中なら起動: bash tools/vmware-sandbox/sandbox.sh start
  3. ユーザーに「解析対象のバイナリパスを教えてください」と質問
  4. ネットワーク隔離: bash tools/vmware-sandbox/sandbox.sh net-isolate（手動フローのみ必要。analyze は自動）
  5. 指示に応じて解析コマンドを構築・実行
     - **推奨**: analyze ワンコマンド（net-isolate/revert/HollowsHunter を自動実行）
     - 手動フロー（詳細制御が必要な場合）: SKILL.md本文の「手動解析手順」（Step 1〜6）を参照
...
**analyze がエラー終了した場合（ファイル不在・コピー失敗等）は自動 revert は走らない → 手動で `bash tools/vmware-sandbox/sandbox.sh revert` を実行すること。**
