...
bash tools/ghidra-headless/ghidra.sh maldev-detect <binary>             # オペレータ手法検出（精密、analyze-full 後）
bash tools/ghidra-headless/ghidra.sh maldev-detect scan-binary <path>   # 生バイナリ直接スキャン（Ghidra 不要・5秒）
bash tools/ghidra-headless/ghidra.sh maldev-detect list                 # 検出可能 15 テクニックのカタログ
bash tools/ghidra-headless/ghidra.sh pe-fallback-extract <binary>       # Ghidra非依存のstrings/imports抽出（IOC/分類のfallback入力）
bash tools/ghidra-headless/ghidra.sh analyze-full <binary>              # フルパイプライン（Phase 0-7）
bash tools/ghidra-headless/ghidra.sh analyze-full --zip-password PW <file.zip>  # ZIPアーカイブ対応（container内展開）
```

**maldev-detect の使い分け:**
- `maldev-detect <binary>` — analyze-full 後の精密検出（imports / strings / decompiled C をすべて使う）
- `maldev-detect scan-binary <path>` — サンプル受領直後の即時トリアージ（Ghidra 待たない、5 秒以内）
- `maldev-detect list` — 15 テクニックの ATT&CK マッピングを確認
- 共通フラグ: `--min-severity high` / `--json-only`

f00crew 0x33 系のオペレータ手法を検出（実装は 15 テクニック = PEB walking / ROR13 hash / FNV-1a/djb2 hash / Process Hollowing / Early Bird APC / NtCreateThreadEx / Reflective DLL / Module Stomping / kernel callback / DKOM token-steal / Driver self-hide / Direct syscall(Hell's Gate) / multi-layer crypto / inline AES / IAT hooking）。出力は `tools/ghidra-headless/output/<binary>_maldev.json` に severity / confidence / 証拠 / ATT&CK を記録。同数の YARA ルール（`yara-rules/custom/maldev_techniques.yar`、15 ルール）と相補的。
...
**Phase 5 失敗時の自動フォールバック（KB-24）**: Ghidra スクリプトが失敗すると `pe_fallback_extract.py` が自動実行され、pefile ベースで `_pe_strings.txt` / `_pe_imports.txt` を生成する。これにより Phase 6 (IOC) / Phase 7 (分類) は Ghidra 非依存で続行可能。

**ZIP パスワード付きアーカイブ対応**: `analyze-full --zip-password infected sample.zip` でコンテナ内 7z 展開 → 解析。展開後は最大サイズのファイルを自動選択。
...
Go バイナリは標準 Ghidra 解析で strings/imports が極端に少なくなる（48MB バイナリで 374 文字列のみ等）。Go が文字列を PE string table ではなく独自のシンボルテーブル (gopclntab) に格納するため。

### 検出方法
- pe_triage.py: `.symtab` / `.gopclntab` セクション、`\xff Go build ID:` マーカー
- DiE: "Go" コンパイラ検出
- 48MB+ の Windows EXE でインポートが極少（KERNEL32 + MSVCRT のみ等） → Go の可能性大

### 推奨ワークフロー
1. `pe-triage` で Go 検出 → `analyze-full` を実行するが **strings/imports 結果は参考程度**
2. Docker コンテナ内で raw strings 抽出（IOC 発見の主力）:
   ```bash
   bash tools/ghidra-headless/ghidra.sh decrypt <file.enc.gz>
   docker exec ghidra-headless strings -n 8 /tmp/<binary> | grep -iE "http|api\.telegram|sql|vmware|Processor|Stealer"
   ```
3. Go シンボルから構造体・メソッド名を抽出（`main.*Processor`, `main.*SendTo*` 等）
4. YARA / CAPA は Go バイナリでも有効（PE ヘッダは存在する）
5. Ghidra decompile は Go ランタイムのノイズが大きいため、xref の多い関数に絞る

### Go Stealer の典型パターン
- エクスポートに `_cgo_dummy_export` / `go*CB` / `*Trampoline` → CGo 使用
- `*Processor` 型（`ChromeBasedProcessor` 等）→ モジュール式 Stealer
- `api.telegram.org/bot%s/%s` → Telegram Bot API exfil
- `SELECT host_key, name, path, encrypted_value` → Cookie/パスワード窃取 SQL
...
| PE Fallback | `<binary>_pe_strings.txt`, `<binary>_pe_imports.txt` |
...
| **24** | **Ghidra スクリプト陳腐化問題**: scripts/ を volume mount して即時反映。Phase 5 失敗時は `pe_fallback_extract.py` が自動生成する `_pe_strings.txt` / `_pe_imports.txt` で IOC/分類が続行。YARA 出力は `/analysis/output/` にもコピーし `output ls` で参照可能に。ZIP は `--zip-password` で container 内 7z 展開。 |
| **25** | **MSI/DLLサイドローディング解析パターン** (Beagle Campaign 2026): pe_triage.py が MSI を検出→"MSI_INSTALLER" verdict。analyze-full は MSI を 7z 展開→内包 PE を個別 YARA/CAPA→最大 PE を Ghidra 解析。msi_extract.py で OLE テーブルパース（File Table / CustomAction / ProductName）。Delphi/FMX バイナリは RTTI (`FMX.Controls.*`, `TMethodImplementationIntercept`) から内部構造復元可能。HTML Content Padding (> 5MB .text + HTML tags) は YARA を完全回避するため RTTI/import ベース検出が必要。DoH URL (`cloudflare-dns.com/dns-query` 等) がバイナリ内にある場合は `sandbox.sh capture-c2` を使用（FakeNet DNS listener 無効）。 |
