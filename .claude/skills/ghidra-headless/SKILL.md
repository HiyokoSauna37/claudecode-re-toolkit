...
bash tools/ghidra-headless/ghidra.sh yargen <binary> [rule_name] # YARAルール自動生成（yarGen）
...
- `yargen`: yarGenでマルウェアサンプルから独自YARAルールを自動生成。正規ソフトの文字列を自動除外し、マルウェア固有のシグネチャを抽出。解析→ルール生成→次回検知のサイクルに活用
